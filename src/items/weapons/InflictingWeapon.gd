...
func on_attack_hit(target_enemy: CombatEntity) -> void:
    if randf() < effect_apply_chance:
        ...
        LogManager.log_damage("Your %s inflicts %s!" % [name, inflicted_effect.get_effect_name()], false)
        target_enemy.apply_status_effect(inflicted_effect)

func get_description() -> String:
    return "%s%% chance to inflict %s on hit." % [int(effect_apply_chance * 100), status_effect.get_effect_name() if status_effect else "No Effect"]
