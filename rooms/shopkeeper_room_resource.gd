...
func build_actions(_actions_grid: GridContainer, _room_screen: RoomScreen) -> void:
    ...
    add_action_button(_actions_grid, "Talk to %s" % shopkeeper_name, _on_talk_to_shopkeeper.bind(_room_screen))

func _on_talk_to_shopkeeper(room_screen: RoomScreen) -> void:
    # Show the shopkeeper popup
    var shopkeeper_popup = load("res://popups/ShopkeeperPopup.tscn").instantiate()
